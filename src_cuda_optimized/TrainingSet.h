...
template <typename FeatureType> class TrainingSet
{
  public:
	...
	virtual void getFlattenedFeatures(uint16_t imageId, FeatureType **out_features, int *out_nbChannels) const
	{
		...
		FeatureType *flat = (FeatureType *)hostAlloc(sizeof(FeatureType)*(this->iWidth)*(this->iHeight)*(this->nChannels));
		if (flat == NULL)
		{
	...
	virtual void getFlattenedIntegralFeatures(uint16_t imageId, FeatureType **out_features_integral, int16_t *out_w, int16_t *out_h) const
	{
		...
		FeatureType *flat = (FeatureType *)hostAlloc(sizeof(FeatureType)*w*h*(this->nChannels));
		if (flat == NULL)
		{
