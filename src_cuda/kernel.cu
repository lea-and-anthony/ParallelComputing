...
void kernel(Sample<FeatureType> &sample, TNode<SplitData<FeatureType>, Prediction> *allNodesArray, FeatureType *features, FeatureType *features_integral, int idRoot, int16_t height, int16_t width, int16_t height_integral, int16_t width_integral, size_t numLabels, int lPXOff, int lPYOff, unsigned int *result)
{
	...
	vector<uint32_t>::const_iterator prediction = predictNoPtr(sample, allNodesArray, features, features_integral, idRoot, height, width, height_integral, width_integral);

	for (int y = (int)sample.y - lPYOff; y <= (int)sample.y + lPYOff; ++y)
	{
		for (int x = (int)sample.x - lPXOff; x <= (int)sample.x + lPXOff; ++x, ++prediction)
		{
			if (*prediction >= numLabels)
			{
			...
			{
				result[(*prediction) * height * width + y * width + x]++;
			}
		}
	}
}
