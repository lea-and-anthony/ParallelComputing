...
using namespace vision;
	...
	__host__ void cudaDisplayError(char *functionName, cudaError_t cudaStatus)
	{
...
#pragma warning (pop)
	...
	__host__ bool canMapHostMemory()
	{
	...
	__host__ void* hostAlloc(size_t size)
	{
		void *hostPointer;
		cudaError_t cudaStatus;
		cudaStatus = cudaHostAlloc(&hostPointer, size, cudaHostAllocMapped);
		
		if (cudaStatus != cudaSuccess)
		{
			cudaDisplayError("cudaHostAlloc", cudaStatus);
			return NULL;
		}
		return hostPointer;
	}

	__host__ void hostFree(void *hostPointer)
	{
		cudaFreeHost(hostPointer);
	}

	__host__ void hostGetDevicePointer(void** devicePointer, void* hostPointer)
	{
		cudaError_t cudaStatus = cudaHostGetDevicePointer(devicePointer, hostPointer, 0);
		if (cudaStatus != cudaSuccess)
		{
			cudaDisplayError("cudaHostGetDevicePointer", cudaStatus);
		}
	}
